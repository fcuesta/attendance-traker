...
import { getSheetMatrix, normalizeHeaderDate, pickDefaultDateIso } from '@/lib/sheets'
...
export async function GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      dates: headerDates.filter((d) => d.iso).map((d) => ({ label: d.label, iso: d.iso! })),
      defaultIso,
    })
  } catch (e: any) {
    return NextResponse.json({ error: e.message || 'Failed to fetch dates' }, { status: 500 })
  }
}
