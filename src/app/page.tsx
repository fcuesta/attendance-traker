...
export default function Page() {
  ...
  React.useEffect(() => {
    fetch('/api/teams')
      .then((r) => r.json())
      .then((data) => {setTeams(data.teams); if (defaultTeam && data.teams.find(t => t.name === defaultTeam)) setTeam(defaultTeam);})
  }, [])
    ...
    <main className="flex flex-col gap-4">
      ...
      <section className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <Select
          ...
          value={team}
          onChange={(v) => {setTeam(v); localStorage.setItem("defaultTeam", v)}}
