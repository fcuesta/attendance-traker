...
export function normalizeHeaderDate(cell: string): {
  iso: string | null;
  label: string;
} {
  const label = cell;
...
export function pickDefaultDateIso(
  headers: { iso: string | null; label: string }[]
): string | null {
  const todayIso = new Date().toISOString().slice(0, 10);
  const valid = headers.filter((h) => !!h.iso) as {
    iso: string;
    label: string;
  }[];
